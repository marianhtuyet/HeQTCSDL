...
----------------
CREATE FUNCTION AUTO_MaHocSinh()
RETURNS INT
AS
BEGIN
	DECLARE @ID INT
	IF (SELECT COUNT(MaHS) FROM dbo.HocSinh) = 0
		SET @ID = 0
	ELSE
	BEGIN
		SELECT @ID = MAX(MaHS)+1 FROM dbo.HocSinh
	END	
...
BEGIN
	DECLARE @ID VARCHAR(5)
	IF (SELECT COUNT(MaLop) FROM dbo.LopHoc) = 0
		SET @ID = '0'
	ELSE
		SELECT @ID = MAX(RIGHT(MaLop, 3)) FROM dbo.LopHoc
		SELECT @ID = CASE
...
GO
-------------------
CREATE FUNCTION AUTO_MaGiaoVien()
RETURNS INT
AS
BEGIN
	DECLARE @ID INT
	IF (SELECT COUNT(MaGV) FROM dbo.GiaoVien) = 0
		SET @ID = 0
	ELSE
		SELECT @ID = MAX(MaGV)+1 FROM dbo.GiaoVien
	RETURN @ID
END
GO
----------------
...
CREATE TABLE GiaoVien(
	MaGV INT PRIMARY KEY CONSTRAINT idGiaoVien DEFAULT dbo.AUTO_MaGiaoVien(),
	TenGV NVARCHAR(50),
	NgaySinh SMALLDATETIME,
	DiaChi NVARCHAR(50),
	SDT NVARCHAR(11),
